...
class TestPurchaseInvoiceUpdateStock(FrappeTestCase):
	...
	def test_a_service_only_order_is_left_alone(self):
		...
		service = make_item(
			"SF Test Service Item", properties={"is_stock_item": 0, "item_group": "Products"}
		).name
		po = self.make_order(item_code=service, qty=1, rate=50)
		pi = make_purchase_invoice(po.name)
		self.assertEqual(int(pi.update_stock or 0), 0, "there is no stock to post")

	def test_it_never_overrides_a_document_that_already_says_yes_or_no(self):
		po = self.make_order()
		pi = make_purchase_invoice(po.name)
		pi.update_stock = 0
		# the tick happens at mapping time only, so a second pass changes nothing
		self.assertFalse(set_update_stock(pi))
		self.assertEqual(int(pi.update_stock or 0), 0, "a buyer who unticked it keeps that")
...
class TestSellingHistory(FrappeTestCase):
	...
	def test_it_reports_what_the_item_sold_for(self):
		...
		self.assertAlmostEqual(mine[0]["rate"], 150, places=2)

		summary = {row["item_code"]: row for row in data["summary"]}
		self.assertIn(self.item_code, summary)
		self.assertAlmostEqual(summary[self.item_code]["last_rate"], 150, places=2)
